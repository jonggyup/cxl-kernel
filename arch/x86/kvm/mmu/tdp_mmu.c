                                  ...
                                  bool is_huge)
{
    ...
    const gfn_t walk_lo   = ALIGN_DOWN(start_gfn, KVM_PAGES_PER_HPAGE(PG_LEVEL_2M));
    const gfn_t walk_hi   = ALIGN(end_gfn,   KVM_PAGES_PER_HPAGE(PG_LEVEL_2M));

    lockdep_assert_held_write(&kvm->mmu_lock);

    /* Only split to 4K and track read-hot when !is_huge */
    if (!is_huge)
        kvm_mmu_try_split_huge_pages(kvm, slot, start_gfn, end_gfn, PG_LEVEL_4K);
    ...
    for_each_valid_tdp_mmu_root_yield_safe(kvm, root, slot->as_id) {
        ...
        tdp_root_for_each_leaf_pte(iter, root, walk_lo, walk_hi) {
            u64 new_spte, clear_mask = 0;
            gfn_t leaf_start, leaf_end;
            bool was_accessed, was_dirty;

            if (!is_shadow_present_pte(iter.old_spte))
                continue;

            leaf_start = max(iter.gfn, start_gfn);
            leaf_end   = min(iter.gfn + KVM_PAGES_PER_HPAGE(iter.level), end_gfn);
            if (leaf_start >= leaf_end)
                continue;

            was_accessed = is_accessed_spte(iter.old_spte);
            was_dirty    = is_dirty_spte(iter.old_spte);

            if ((!was_accessed && !was_dirty) || (is_huge && !was_dirty))
                continue;

            /* ---- READ-HOT (A-bit) ----
             * Requirement: ONLY track when !is_huge. We also avoid clearing A when is_huge,
             * so A continues to accumulate until a fine-grained pass occurs.
             * If you want "strict read-only hot", use (was_accessed && !was_dirty).
             */
            if (!is_huge && was_accessed) {
                if (iter.level == PG_LEVEL_4K) {
                    idx = leaf_start - start_gfn;
                    set_bit(idx, slot->fmsync_read_bitmap);
                } else { /* residual >4K leaf */
                    idx = leaf_start - start_gfn;
                    bitmap_set(slot->fmsync_read_bitmap, idx, leaf_end - leaf_start);
                }
                clear_mask |= shadow_accessed_mask; /* clear A only in !is_huge mode */
            }

            /* ---- DIRTY (D-bit) ---- (track regardless of is_huge) */
            if (was_dirty) {
                if (is_huge) {
                    idx = (leaf_start - start_gfn) >> (HPAGE_SHIFT - PAGE_SHIFT);
                    set_bit(idx, slot->fmsync_dirty_bitmap);
                } else if (iter.level == PG_LEVEL_4K) {
                    idx = leaf_start - start_gfn;
                    set_bit(idx, slot->fmsync_dirty_bitmap);
                    ...
                    idx = leaf_start - start_gfn;
                    bitmap_set(slot->fmsync_dirty_bitmap, idx, leaf_end - leaf_start);
                }
                clear_mask |= shadow_dirty_mask | PT_WRITABLE_MASK; /* clear D & drop W */
            }

            if (clear_mask) {
                new_spte = iter.old_spte & ~clear_mask;
                if (tdp_mmu_set_spte_atomic(kvm, &iter, new_spte))
                    kvm_flush_remote_tlbs_range(kvm, iter.gfn,
                            KVM_PAGES_PER_HPAGE(iter.level));
                flush_needed = true;
